...
if uploaded_file is not None:
    ...
    st.plotly_chart(px.bar(top_ips, x='Source IP', y='Count'), use_container_width=True)

    st.subheader("📈 Detection Rate")
    rate = filtered_df['Label'].value_counts(normalize=True).reset_index()
    rate.columns = ['Label', 'Percentage']
    st.plotly_chart(px.pie(rate, values='Percentage', names='Label'), use_container_width=True)

    st.subheader("📆 Intrusion Events Over Time")
    time_df = malicious_df.dropna(subset=['Timestamp']).groupby(malicious_df['Timestamp'].dt.floor('H')).size().reset_index(name='Count')
    st.plotly_chart(px.line(time_df, x='Timestamp', y='Count'), use_container_width=True)
    ...
    if 'Destination IP' not in df.columns:
        ...
        if st.button("🔍 Run IP Reputation Check (Top 30 Malicious IPs)"):
            ...
            ip_list = mal_df['Destination IP'].value_counts().head(TOP_N).index.tolist()
            results = {}
            ...
            with st.spinner("⏳ Querying AbuseIPDB..."):
                ...
                st.download_button("📥 Download Results as JSON", json_str, file_name="api_enriched_threats.json", mime="application/json")
